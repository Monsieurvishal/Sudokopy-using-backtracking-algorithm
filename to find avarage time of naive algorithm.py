...
def sudoku_generate(matrix, frequency):
    ...
    while not checking_condition(matrix):
        ...
        while (matrix.rows in store):
            ...
            #pass the randomly generated matrix to class Matrix4 as constuctor to the matrix
            time=time+1
...
if __name__ == "__main__": 
    count=0
    total1=0
    while(int(input(' Enter 1 to continue'))==1):
        frequency = 20
        Mat = list()
        num=[1,2,3,4]
        for i in range(0, 4):
        ...
        Matrix,time=sudoku_generate(Matrix, frequency)
        total1=time+total1
        print(int(total1/count))
        Matrix.show()
